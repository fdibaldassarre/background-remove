...
class ImageSplitter():
    ...
    def upscaleMatrix(self, kernelIn, newshape):
        '''
        w, h = newshape
        #kernelOut = numpy.zeros((outKSize, outKSize), dtype='float32')
        kh, kw = kernelIn.shape
        # Prepare data
        x = numpy.arange(kw)
        y = numpy.arange(kh)
        z = kernelIn
        # Linspaces
        xx = numpy.linspace(x.min(), x.max(), w)
        yy = numpy.linspace(y.min(), y.max(), h)
        # Interpolate
        newKernel = interpolate.RectBivariateSpline(x,y,z, kx=2,ky=2)
        return newKernel(xx, yy)
        '''
        # I should never use this method
    ...
    def splitImage(self, img, size, padding=False, stride=None):
        '''
            ...
            :param size: size of the regions to be returned
            :param padding: add padding to process all the image or not
            :param stride: tuple with the stride between two nearby regions
        ...
        base_img = Image.new(img.mode, base_size, color="black")
